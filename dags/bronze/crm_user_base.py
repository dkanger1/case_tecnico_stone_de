...
FILE_CONFIG = {
    "file": "/opt/airflow/data/crm_user_base.csv",
    "table": "crm_user"
}

# Colunas esperadas (opcional - descomente e ajuste conforme necessário)
# EXPECTED_COLUMNS = ["user_id", "name", "email", "age"]

@dag(
    schedule=None,
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=['duckdb', 'ingestion', 'bronze', 'crm'],
    description='Ingestão bronze: CRM User Base CSV -> tabela crm_user no DuckDB',
    max_active_tasks=1
)
def ingestion_crm_user_duckdb():

    @task
    def ingest_crm_user():
        """
        Task de ingestão do arquivo CRM User Base para a camada bronze.
        """
        try:
    ...
    # Executar task
    ingest_task = ingest_crm_user()

# Instanciando a DAG
dag_exec = ingestion_crm_user_duckdb()